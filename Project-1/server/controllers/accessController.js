...
export const accessLevelHandler = async (req,res) => {
    try{
        ...
        if(!userNotification){
            await Notifications.create({id: payload.userId, notifications:[`Request for ${payload.requestingAccess} has been sent`]})
        }
        else{
            if(userNotification.notifications.length >= 5){
                ...
                existingNotifications.unshift(`Request for ${payload.requestingAccess} has been sent`)
